---
title: "Chapter 7 - Gene Expression Analysis - Microarrays"
...
If you only have an ExpressionSet already, RMA has likely already been applied 
to the data. Use exprs() to access normalized expression values:

```{r If You Only Have ExpressionSet}
...
Applying rma() again is not necessary and will error out because ExpressionSet is 
not compatible with rma.
...
* __treatment samples__

GSM1394594 GSM1394595 GSM1394596 GSM1394597 GSM1394598 GSM1394599

* __negative control samples__

GSM1394600 GSM1394601 GSM1394602 GSM1394603 GSM1394604 GSM1394605 

```{r 1. C. I. Design Matrix}
...
```{r 1. C. II. Fit Model and Conduct Analysis}
# Load the Libraries
library(limma)
...
```{r 1. C. III. Result Interpretation - Volcano Plot}
# Volcano Plot
...
```{r 1. C. III . Result Interpretation - Enhanced Volcano Plot}
# Load Library
...
EnhancedVolcano(volcano_data, 
                ...
                FCcutoff = 1.0,
                # Change axis limits
                xlim = c(-1.05, 1.5),     # X-axis: log2 fold change from -1.05 to +1.5
                ylim = c(0, 11.6),      # Y-axis: -log10(p-value) from 0 to 11.6
                title = "Volcano Plot",
...
```{r 1. D. Functional Enrichment Analysis}
...
the data using box plots which help identify any outliers:

```{r 2. D. Quality Assessment}
...
```{r 2. E. I. Perform background correction on Affymetrix micro-array}
...
```{r 2. E. II. Perform background correction on Illumina micro-array}
...
#### 2. F. I. Normalize the data using RMA 
...
```{r 2. F. I. Normalize the data using RMA }
...
```{r 2. H. Filtering and Transformation}
...
```{r 3. B. Installation and Loading of limma}
...
```{r 3. D. Normalization and Transformations}
# Load edgeR library
...
```{r 3. F. Empirical Bayes Moderation}
fit <- eBayes(fit)
fit
...
```{r 3. G. Identifying Differentially Expressed Genes}
...
#### 3. H. I. Visualization - volcano plot

```{r 3. H. I. Visualization - volcano plot}
# Install package
...
#### 3. H. II. Visualization - Heat Map

Heat maps can be created to visualize the expression patterns of DEGs across 
samples:

```{r 3. H. II. Visualization - Heat Map}
# load Library
library(pheatmap)
